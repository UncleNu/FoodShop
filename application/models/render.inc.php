...
if ($pages > 1 || $alwaysShowPagination === true) {
    ...
    <ul class="pagination font-th">
        ...
        if($pages == $current){
            ?>
            <li><a href="<?php echo $target;?>?page=1"><?php echo (_LANG_DIGI_ == 'th') ? 'หน้าแรก': 'First'?></a></li>
            <li class="<?= implode(' ', $classes) ?>"><a href="<?= ($href) ?>"><?php echo (_LANG_DIGI_ == 'th') ? 'ก่อนหน้า': 'Previous'?></a></li>
            <?php
        }
        /**
        ...
        if (!$clean) {
            ...
            for ($x = 0; $x < $leading; ++$x) {
                ...
                $href = preg_replace(
                    ...
                    ?>
                    <li><a data-pagenumber="<?= ($current + $x - $leading) ?>" href="<?= ($href) ?>"><?= ($current + $x - $leading) ?></a></li>
                    <?php
                }

            // print current page
                ?>
                <li><a data-pagenumber="<?= ($current) ?>" class="active" href="#"><?= ($current) ?></a></li>
                <?php
            // generate/render trailing crumbs
                ...
                // class/href setup
                    ...
                    $href = preg_replace(
                        ...
                        ?>
                        <li><a data-pagenumber="<?= ($current + $x + 1) ?>" href="<?= ($href) ?>"><?= ($current + $x + 1) ?></a></li>
                        <?php
                    }
                }
        ...
        if($pages != $current){
            ?>
            <li><a href="<?= ($href) ?>"><?php echo (_LANG_DIGI_ == 'th') ? 'ถัดไป': 'Next'?></a></li>
            <li><a href="<?php echo $target;?>?page=<?php echo $pages;?>"><?php echo (_LANG_DIGI_ == 'th') ? 'สุดท้าย': 'End'?></a></li>
            <?php
        }
        ?>
    </ul>
